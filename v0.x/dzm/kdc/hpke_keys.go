...
func GetKdcHpkeKeypair(privKeyPath string) (*KdcHpkeKeys, error) {
	...
	if privKeyPath != "" {
		if keys, err := loadKdcHpkeKeypair(privKeyPath); err == nil {
			...
			log.Printf("KDC: ERROR: Cannot decrypt bootstrap requests without the correct HPKE private key!")
			log.Printf("KDC: ERROR: The bootstrap blob was encrypted with a different public key.")
			...
			return nil, fmt.Errorf("failed to load HPKE keypair from %s: %v (cannot decrypt bootstrap requests without the correct key)", privKeyPath, err)
		}
	}

	// No key path configured
	log.Printf("KDC: ERROR: kdc_hpke_priv_key is not configured in KDC config")
	log.Printf("KDC: ERROR: Cannot decrypt bootstrap requests without HPKE private key!")
	return nil, fmt.Errorf("kdc_hpke_priv_key is not configured in KDC config - required for bootstrap decryption")
	pubKey, privKey, err := hpke.GenerateKeyPair()
	if err != nil {
		return nil, fmt.Errorf("failed to generate HPKE keypair: %v", err)
	}
	...
	if privKeyPath != "" {
		if err := saveKdcHpkeKeypair(keys, privKeyPath); err != nil {
			log.Printf("KDC: Warning: Failed to save HPKE keypair to %s: %v", privKeyPath, err)
			// Continue anyway - keys are in memory
		} else {
			log.Printf("KDC: Saved HPKE keypair to %s", privKeyPath)
		}
	}
...
# This key is used by KDC to decrypt bootstrap requests and other encrypted data.
#
# Public Key: %s
#
%s
